export const Experiences = [
  {
    id: 1,
    role: "Full Stack Developer Intern",
    company: "Ilie Agaue Technologies Pvt. Ltd.",
    location: "Noida, UP",
    period: "Aug 2025 – Feb 2026",
    type: "Internship",
    bullets: [
      "Engineered responsive UI components and integrated complex RESTful APIs using the MERN Stack, reducing page load time by 15%.",
      "Optimized MongoDB queries and schema designs using indexing and aggregation pipelines, improving data retrieval speed by 20%.",
      "Architected secure user authentication workflows utilizing JWT and Role-Based Access Control (RBAC), enhancing application security.",
      "Collaborated in an Agile/Scrum environment, performing code reviews and maintaining version control via Git/GitHub.",
    ],
    tags: ["React.js", "Node.js", "MongoDB", "Express", "JWT", "RBAC", "Git"],
  },
];
